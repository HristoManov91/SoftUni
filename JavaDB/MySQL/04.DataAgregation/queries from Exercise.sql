...
SELECT `department_id` , AVG(`salary`) AS `avg_salary` FROM `exercise13`
GROUP BY `department_id`
ORDER BY `department_id`;
...
SELECT `department_id` , (
	SELECT DISTINCT `salary` FROM `employees` AS emp2
    WHERE emp1.`department_id` = emp2.`department_id`
    ORDER BY emp2.`salary` DESC
    LIMIT 1 OFFSET 2
) AS `ths`  FROM `employees` AS emp1
GROUP BY `department_id`
