...
public class BookServiceImpl implements BookService {
    ...
    @Override
    public List<Book> getAllBooksFromAuthorGeorgePowell() {
        Author author = authorService.getAuthorByName();
        return bookRepository.findAllByAuthorEqualsOrderByReleaseDateDescTitle(author);
    }
    ...
    private Set<Category> getRandomCategories() {
        ...
        for (int i = 1; i <= bound; i++) {
            ...
            result.add(categoryService.getCategoryById((long) categoryId));
        }
        return result;
    }
    ...
    private Author getRandomAuthor() {
        Random random = new Random();
        int randomId = random.nextInt(this.authorService.getCountAuthors()) + 1;
