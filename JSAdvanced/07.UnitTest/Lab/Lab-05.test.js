const {isSymmetric} = require('./Lab-05');
const {expect} = require('chai');

describe('Test isSymmetric function', () => {
    it('Test with valid input', () => {
        expect(isSymmetric([1, 2, 3, 4, 5, 4, 3, 2, 1])).to.be.true;
        expect(isSymmetric(['a', 'b', 'c', 'b', 'a'])).to.be.true;
        expect(isSymmetric([])).to.be.true;
    })
    it('Test with non array return false', () => {
        expect(isSymmetric({})).to.be.false;
        expect(isSymmetric('a')).to.be.false;
        expect(isSymmetric(1)).to.be.false;
        expect(isSymmetric(undefined)).to.be.false;
        expect(isSymmetric(new Date())).to.be.false;
        expect(isSymmetric(true)).to.be.false;
    })
    it('Test with different array return false', () => {
        expect(isSymmetric([1, 2, 3])).to.be.false;
        expect(isSymmetric(['arra' , 'asfa' , 'asfa' , 'a'])).to.be.false;
