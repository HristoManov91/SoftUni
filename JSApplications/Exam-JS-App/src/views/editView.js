...
import {animalIsInvalid} from "../utils/validators.js";

const editTemplate = (submitHandler , animal) => html`
    <section id="editPage">
...
export const editView = (ctx) => {
    ...
    const submitHandler = (e) => {
        ...
        if (animalIsInvalid(animalData)){
            alert('All fields should be filled');
            return;
        }

        animalService.edit(animalId , animalData)
            .then(() => {
                ctx.page.redirect(`/pets/${animalId}`)
            })
    };

    animalService.getAnimal(animalId)
        .then(animal => {
            ctx.render(editTemplate(submitHandler , animal));
        })
}
